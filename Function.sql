...
-- RANK() ROW_NUMBER() 차이
 결과 집합의 행에 순위를 할당하는 데 사용.

RANK()함수
 동일한 값이 있는 경우 동일한 순위 부여.
 동일한 값이 여러번 나타나면 동일한 순위가 여러번 반복
 예를 들어 동일한 값을 가진 두번째와 세번째 행에 둘다 2위 할당 결과 집합에서 중간에 순위를 건너 뛸 수 있음.

ROW_NUMBER()함수
 동일한 값이 있더라도 각 행에 고유한 순위를 부여.
 동일한 값이 있는 경우에도 각 행에 대해 중복되지 않는 순위를 할당.
 예를 들어 동일한 값을 가진 두번째와 세번째 행에 대해 각각 2위와 3위 할당 결과 집합에서 순서는 항상 연속적.

ROWNUM
 데이터베이스에서 사용할 수 있는 가상 열 중 하난.
 검색 결과 집합의 각 행에 할당된 고유 번호를 나타냄.
 결과 집합의 특정 행을 선택하거나 행에 번호를 할당하는 데 사용.
 특징 : 검색 결과 집합을 처리하는 동안 생성됨.
  결과 집합에 대한 각 행에 할당.
  1부터 시작하여 각 행에 연속된 정수값을 부여.
